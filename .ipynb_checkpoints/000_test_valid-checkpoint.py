...
def run_prediction(model_name, test_csv, test_npz_dir, annot_fn, ontology='mf', 
    ...
    if output_file is None:
        output_file = f"results/cafa/{ontology}_predictions.pckl"
...
def get_default_paths(ontology, dataset_type='valid', run_number=1):
    ...
    return {
        'txt_file': f'cafa/{ontology_upper}_{dataset_info["txt_suffix"]}',
        'annot_fn': f'cafa_deepfri/cafa_{ontology_lower}_annot.tsv',
...
def main():
    ...
    parser.add_argument('--model_name', type=str, required=True,
                       help='模型名称（例如: trained_models/CafaModel_MF）')
    ...
    parser.add_argument('--txt_file', type=str, default=None,
                       help='蛋白质列表TXT文件路径（默认: cafa/{ONTOLOGY}_{dataset_type}_proteins.txt）')
    parser.add_argument('--test_npz_dir', type=str, required=True,
                       help='NPZ文件目录')
    parser.add_argument('--annot_fn', type=str, default=None,
                       help='注释文件路径（默认: cafa_deepfri/cafa_{ontology}_annot.tsv）')
    parser.add_argument('--output_dir', type=str, default='results/cafa',
                       help='输出目录')

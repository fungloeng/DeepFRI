...
def load_GO_annot(filename):
    # Load GO annotations (supports optional PF)
...
def get_batched_dataset(filenames, batch_size=64, pad_len=1000, n_goterms=347, channels=26, gcn=True, cmap_type='ca', cmap_thresh=10.0, ont='mf'):
    ...
    # 估算：每个样本约 pad_len^2 * 4 bytes (float32) for contact map + pad_len * 26 * 4 bytes for sequence
    # 假设可用内存约 8GB，保守估计每个样本约 (pad_len^2 + pad_len*26) * 4 bytes
    if pad_len > 2000:
        # 对于很大的 pad_len，使用较小的 buffer
        buffer_size = max(100, min(500, 2000 - (pad_len - 1000) // 10))
    elif pad_len > 1500:
        buffer_size = max(200, min(1000, 2000 - (pad_len - 1000) // 5))
    elif pad_len > 1000:
        buffer_size = max(500, min(1500, 2000 - (pad_len - 1000) // 2))
    else:
        buffer_size = 2000 + 3*batch_size
    
    # Randomizes input using a dynamically sized buffer
